class Solution {
    ...
    //     return solve(ip,0,map,dp);
    // }
    static int solve(List<Integer> ip , int i , Map<Integer, Integer> map , Integer [] dp){
        if(i>=ip.size()){
            return 0;
        }
        int next =i+1;
        if(next < ip.size() && ip.get(next) == ip.get(i)+1){
            next++;
        }
        if(dp[i]!=null) return dp[i];
        int include = ip.get(i) * map.get(ip.get(i)) + solve(ip,next,map,dp);
        int exclude = solve(ip,i+1,map,dp);
